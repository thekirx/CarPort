...
export default function HomePage() {
  return <>
    ...
    <section className="section concrete"><div className="container section-heading"><div><span className="eyebrow">REAL SHOP. REAL WORK.</span><h2>Built at Carport.</h2></div><Link className="text-link" href="/builds">View the gallery <ArrowRight size={16} /></Link></div><div className="container build-grid">{builds.slice(0, 4).map((build) => <figure key={build.image}><Image src={build.image} alt={`${build.vehicle} at Carport Wheels`} fill sizes="(max-width: 700px) 90vw, 25vw" /><figcaption><span className="sample-badge">ILLUSTRATIVE CAPTION</span><b>{build.vehicle}</b><small>{build.detail}</small></figcaption></figure>)}</div></section>
